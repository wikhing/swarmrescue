...
@mcp.tool()
def get_distress_signal() -> str:
    """Scans the global area and returns the X, Y coordinates of any detected distress signals (survivors)."""
    survivors = [a for a in sim.agents if type(a).__name__ == "SurvivorAgent"]
    if survivors:
        return f"Distress signal isolated at coordinate {survivors[0].pos}."
    return "No distress signals detected."
...
def get_active_drones() -> str:
    ...
    for drone in sim.agents:
        if type(drone).__name__ == "DroneAgent":
...
def thermal_scan(drone_id: int) -> str:
    ...
    for drone in sim.agents:
        if type(drone).__name__ == "DroneAgent" and drone.unique_id == drone_id:
            ...
            cell_contents = sim.grid.get_cell_list_contents([drone.pos])
            has_survivor = any(type(obj).__name__ == "SurvivorAgent" for obj in cell_contents)
